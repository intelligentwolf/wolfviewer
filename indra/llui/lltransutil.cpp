...
bool LLTransUtil::parseStrings(const std::string& xml_filename, const std::set<std::string>& default_args)
{
    ...
    if (!success)
    {
        ...
        LL_ERRS() << "Couldn't load string table " << xml_filename << ". Please reinstall the viewer from https://www.wolf-grid.com/index.php?f=osv and ask at https://www.wolf-grid.com/blog/faq.php if the issue persists after reinstall." << LL_ENDL;
        return false;
    }
